...
final class SettingDescriptorTest extends TestCase {
    public function testDeclaredTypesAndBounds(): void {
        $schema=SettingDescriptor::declare(['Section,title','yes'=>'Enabled,bool|1','count'=>'Count,int|0','bounded'=>'Bounded,range,1,20,1|10','decimal'=>'Modifier,floatrange,.1,2,.05|.75','choice'=>'Mode,enum,a,First,b,Second|a','text'=>'Name','amount'=>'Adjustment,amount|-10%','hidden'=>'Read only,viewonly']);
        ...
        foreach ([['yes','true'],['yes','2'],['count','1e2'],['count','2147483648'],['bounded','0'],['bounded','21'],['decimal','INF'],['decimal','2.1'],['choice','c'],['text',str_repeat('x',256)],['amount','1%%'],['amount','1e3%'],['text',[]],['text',"bad\0text"]] as [$key,$value]) {
        ...
        self::assertSame('-10%',SettingDescriptor::value($schema['amount'],'-10%'));
        self::assertSame('25',SettingDescriptor::value($schema['amount'],'25'));
